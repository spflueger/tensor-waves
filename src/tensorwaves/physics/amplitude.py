...
class Intensity(Function):
    ...
    function via `sympy.lambdify`, with many possible evaluation backends
    available.

    Args:
        model: A `~.ModelInfo` instance created via the expertsystem
        backend: A string or mapping passed to the `sympy.lambdify` call as the
          `modules` argument.
